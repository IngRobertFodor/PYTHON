...
try:
    ...
    example1_link = wait.until(EC.presence_of_element_located((By.LINK_TEXT, "Example 1: Element on page that is hidden")))
    example1_link.click()


    # 1.2  This checks 2 displayed texts.    
    displayed_text_one = wait.until(EC.presence_of_element_located((By.TAG_NAME, "h3")))
    displayed_text_two = wait.until(EC.presence_of_element_located((By.TAG_NAME, "h4")))
    print(displayed_text_one.text)
    print(displayed_text_two.text)

    
    # 1.3  This checks hidden <h4> text.
    displayed_text_three = wait.until(EC.presence_of_all_elements_located((By.XPATH, "//*[@id='finish' and @style='display:none']/h4")))
